...
import {MathLib, WAD_INT as WAD} from "./lib/MathLib.sol";
import {ConstantsLib} from "./lib/ConstantsLib.sol";
import {MarketParamsLib} from "../lib/MarketParamsLib.sol";
import {Id, MarketParams, Market} from "../interfaces/ILending8.sol";
...
contract AdaptiveCurveIrm is IAdaptiveCurveIrm {
    ...
    using Lending8MathLib for uint128;
    ...
    /// @notice Constructor.
    /// @param lending8 The address of Lending8.
    constructor(address lending8) {
        require(lending8 != address(0), ErrorsLib.ZERO_ADDRESS);
    ...
    function borrowRate(MarketParams memory marketParams, Market memory market) external returns (uint256) {
        require(msg.sender == LENDING8, ErrorsLib.NOT_LENDING8);
    ...
    function _borrowRate(Id id, Market memory market) private view returns (uint256, int256) {
        ...
        int256 utilizationError = (utilization - ConstantsLib.TARGET_UTILIZATION).wDivToZero(errNormFactor);
        ...
        if (startRateAtTarget == 0) {
            ...
            int256 speed = ConstantsLib.ADJUSTMENT_SPEED.wMulToZero(utilizationError);
        ...
        return (uint256(_curve(avgRateAtTarget, utilizationError)), endRateAtTarget);
    }

    /// @dev Returns the rate for a given `_rateAtTarget` and a `utilizationError`.
    /// The formula of the curve is the following:
    /// r = ((1-1/C)*utilizationError + 1) * rateAtTarget if utilizationError < 0
    ///     ((C-1)*utilizationError + 1) * rateAtTarget else.
    function _curve(int256 _rateAtTarget, int256 utilizationError) private pure returns (int256) {
        // Non negative because 1 - 1/C >= 0, C - 1 >= 0.
        int256 curveCoefficient = utilizationError < 0 ? WAD - WAD.wDivToZero(ConstantsLib.CURVE_STEEPNESS) : ConstantsLib.CURVE_STEEPNESS - WAD;
        // Non negative if _rateAtTarget >= 0 because if utilizationError < 0, curveCoefficient <= 1.
        return (curveCoefficient.wMulToZero(utilizationError) + WAD).wMulToZero(int256(_rateAtTarget));
    }
