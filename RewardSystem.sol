...
contract RewardSystem is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function activateProjectRewards(uint256 _projectId, uint256 _totalInvested) external onlyFundraise {
        ...
        if (_totalInvested > 0) {
            ...
            if(burnPercentage > 0) {
                uint256 burnUSD = (_totalInvested * burnPercentage) / BASIS_POINTS;
                if (burnUSD > usdc.balanceOf(address(this))) {
                    revert("Not enough USDC to burn");
                }
                if (burnUSD > 0) {
                    // Approve Uniswap to spend USDC
                    usdc.approve(address(uniswapRouter), burnUSD);

                    // Buy back tokens from pool (USDC -> Token)
                    address[] memory path = new address[](2);
                    path[0] = address(usdc);
                    path[1] = address(token);
                    uint256[] memory amounts;
                    try uniswapRouter.swapExactTokensForTokens(burnUSD, 0, path, address(this), block.timestamp) returns (uint256[] memory _amounts) {
                        amounts = _amounts;
                    } catch {
                    ...
                    uint256 tokensToBurn = amounts[1];
                    if (tokensToBurn > 0) {
                        Token(token).burn(tokensToBurn);
                    }
                }
            }

            // Mint rewarded tokens to RewardSystem contract
            if(rewardTokensAmount[_projectId] > 0) {
                Token(token).mintReward(address(this), rewardTokensAmount[_projectId]);
            }
        }
    }
    ...
    function _calculateVestingAmountForProject(address _user, uint256 _projectId) internal view returns (uint256) {
        ...
        uint256 totalUnlocked = (refData.totalRewardsTokens * weeksPassed * weeklyUnlock) / BASIS_POINTS;
