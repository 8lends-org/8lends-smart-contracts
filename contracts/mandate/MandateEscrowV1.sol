...
contract MandateEscrowV1 is IMandateEscrowV1 {
    ...
    {
        exposure = IMandateRouter(ROUTER).exposure(address(this), pid);
        cap = (mandateSize() * _projectLimitBps) / BPS;
    ...
    function allocate(uint256 pid, address inviter) external onlyOperator {
        ...
        // the placement's own arguments sit too deep to reach.
        {
            ...
            amount = Math.min(Math.min(want, capacity), freeBalance());
        }
    ...
    function onPayout(
        ...
        // The budget is Fundraise's to work out; this clone only spends it in order.
